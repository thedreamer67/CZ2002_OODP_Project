...
public class StudentManager {
	...
	public void addCourse(DataManager dm) {
		...
		if((c.getNumOfAUs()+this.user.getTotalAUs())>dm.getMaxAU()){
			System.out.println("Addition of this course will exceed the maximum total number of AUs allowed, please drop a course first.");
			return;}
	...
	public void dropCourse(DataManager dm) {
		Scanner sc = new Scanner(System.in);
		this.user.checkRegistered();
		if (this.user.getCourseRegistered().size()==0) {
			return;
		}
		...
		if (ci.getWaitingList().size()>0) {
			updateWaitingList(ci, dm); // once current student drops a course, method is called to register student at the top of the waiting list for this course if waiting list is not empty
		}
	...
	public void dropWaitingList(){
		Scanner sc = new Scanner(System.in);
		this.user.checkWaitingList();
		if (this.user.getWaitingList().size()==0) {
			return;
		}
	...
	public void changeIndex(DataManager dm) {
		...
		if(ciNew.getVacancy()>0){
			...
			this.user.checkRegistered();
			if (ciOld.getWaitingList().size()>0) {
				updateWaitingList(ciOld, dm); // once current student drops a course, method is called to register student at the top of the waiting list for this course if waiting list is not empty
			}
		}
		else{
